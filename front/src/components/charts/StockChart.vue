...
const processData = (rawData) => {
  ...
  return {
    ...
    openPrices: filteredData.map(entry => Number(parseFloat(entry[1]).toFixed(2))),
    highPrices: filteredData.map(entry => Number(parseFloat(entry[2]).toFixed(2))),
    lowPrices: filteredData.map(entry => Number(parseFloat(entry[3]).toFixed(2))),
    closePrices: filteredData.map(entry => Number(parseFloat(entry[4]).toFixed(2)))
  };
};
...
const fetchData = async () => {
  try {
    ...
    const { labels, openPrices, highPrices, lowPrices, closePrices } = processData(parsedData.data);
