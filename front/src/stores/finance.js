...
export const useFinanceStore = defineStore('finance', {
  ...
  actions: {
    async findRates(productId) {
      try {
        const response = await axios.get(`http://0.0.0.0:8000/finlife/${productId}/top_rate_month/`)
    ...
    async fetchBanks() {
      try {
        const response = await axios.get('http://0.0.0.0:8000/finlife/getBankName/')
    ...
    async fetchUserSubscribedProducts() {
      ...
      try {
        const response = await axios.get('http://0.0.0.0:8000/account/profile/', {
          headers: {
    ...
    async fetchProducts() {
      ...
      try {
        // 1. 상품 목록 가져오기
        const response = await axios.get('http://0.0.0.0:8000/finlife/deposit-products/')
        ...
        const productsWithDetails = await Promise.all(products.map(async (item) => {
          ...
          try {
            const optionResponse = await axios.get(`http://0.0.0.0:8000/finlife/${item.fin_prdt_cd}/top_rate_month/`)
    ...
    async fetchProductOptions(productId) {
      try {
        const response = await axios.get(`http://0.0.0.0:8000/finlife/deposit-products-options/${productId}/`)
    ...
    async addProductToAccount(productId) {
      ...
      try {
        const response = await axios.post(
          `http://0.0.0.0:8000/account/add-product/`,
    ...
    async removeProductFromAccount(productId) {
      ...
      try {
        const response = await axios.delete(
          `http://0.0.0.0:8000/account/remove-product/`,
          {
            headers: {
              Authorization: `Token ${authStore.token}`,
            },
